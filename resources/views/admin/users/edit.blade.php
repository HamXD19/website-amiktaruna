...
@section('title', 'Edit Pengguna & Hak Akses')
...
<div class="container-fluid px-0">
    ...
    <div class="mb-4">
        <div class="d-flex align-items-center gap-2 text-muted small mb-1">
            ...
            <span>&bull;</span>
            <span class="text-emerald-700 fw-semibold">Edit Pengguna: {{ $user->name }}</span>
        </div>
        <h1 class="h3 fw-bold text-slate-800 mb-0">Ubah Profil &amp; Hak Akses Pengguna</h1>
        <p class="text-muted small mb-0">Sesuaikan kredensial, peran hirarki, serta daftar menu yang dapat diakses oleh akun ini.</p>
    </div>
                ...
                <div class="card border-0 rounded-4 shadow-sm bg-white p-3 p-md-4 mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="text" name="name" class="form-control border-start-0" placeholder="Contoh: Budi Santoso, S.Kom" value="{{ old('name', $user->name) }}" required>
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="email" name="email" class="form-control border-start-0" placeholder="email@amiktaruna.ac.id" value="{{ old('email', $user->email) }}" required>
                        </div>
                    </div>

                    <div class="alert alert-light border rounded-3 p-2.5 mb-3 small text-muted">
                        <i class="fas fa-circle-info text-primary me-1"></i>
                        Kosongkan password jika tidak ingin mengubah password akun saat ini.
                    </div>

                    <!-- Password (Optional) -->
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-slate-700">Password Baru (Opsional)</label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-muted border-end-0"><i class="fas fa-lock"></i></span>
                            <input type="password" name="password" class="form-control border-start-0" placeholder="Biarkan kosong jika tidak diubah">
                        </div>
                    </div>

                    <!-- Password Confirmation -->
                    <div class="mb-4">
                        <label class="form-label small fw-bold text-slate-700">Konfirmasi Password Baru</label>
                        <div class="input-group">
                            <span class="input-group-text bg-light text-muted border-end-0"><i class="fas fa-shield-check"></i></span>
                            <input type="password" name="password_confirmation" class="form-control border-start-0" placeholder="Ketik ulang password baru">
                        </div>
                    </div>
                    ...
                    <div class="d-flex flex-column gap-2 mb-3">
                        ...
                        <label class="role-option-card p-3 rounded-3 border d-flex align-items-start gap-3 cursor-pointer" id="roleCardAdmin">
                            <input type="radio" name="role" value="admin" class="mt-1" {{ $currentRole === 'admin' ? 'checked' : '' }} onchange="onRoleChange('admin')">
                            <div>
                        ...
                        <label class="role-option-card p-3 rounded-3 border d-flex align-items-start gap-3 cursor-pointer" id="roleCardSuper">
                            <input type="radio" name="role" value="super_admin" class="mt-1" {{ $currentRole === 'super_admin' ? 'checked' : '' }} onchange="onRoleChange('super_admin')">
                            <div>
                                <div class="fw-bold text-slate-800 d-flex align-items-center gap-1.5">
                        ...
                        @foreach($availableMenus as $key => $menu)
                            <div class="col-12 col-sm-6">
                                <label class="menu-check-card p-3 rounded-3 border h-100 d-flex align-items-start gap-3 cursor-pointer">
                                    <input type="checkbox" name="permissions[]" value="{{ $key }}" class="menu-checkbox form-check-input mt-1 flex-shrink-0" {{ in_array($key, $userPermissions) || $currentRole === 'super_admin' ? 'checked' : '' }}>
                                    <div class="min-w-0">
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const checkedRole = document.querySelector('input[name="role"]:checked')?.value || '{{ $user->role }}';
